---
sidebar_position: 99
title: Diagram inventory & prioritisation
audience: Tech leads, PM, doc maintainers
summary: Catalog of every workflow / feature flow across the three projects that is a candidate for a Mermaid diagram, prioritised P0/P1/P2. Use this to plan authoring batches.
---

# Diagram inventory & prioritisation

Generated from a controller-walk of the three repos plus the existing
doc pages. Numbers reflect today's reality, not aspiration.

Heuristic: an action becomes a candidate row only when it has a
**workflow shape** — multi-table writes, status transitions, queue or
external-call dispatch, signature verification, geofence checks,
cross-project round-trips. Pure list/view/CRUD endpoints and form
handlers are excluded.

## Summary

- **sd-main** — 95 candidate flows (28 P0, 41 P1, 26 P2)
- **sd-billing** — 32 candidate flows (12 P0, 12 P1, 8 P2)
- **sd-cs** — 18 candidate flows (5 P0, 8 P1, 5 P2)
- **Cross-project integration flows** — 10 flows (10 P0)
- **Total candidates: 155**, of which 13 already have a Mermaid block
  in the docs.

## Already done

Flows that already have an inline Mermaid block somewhere in the docs.

| # | Flow | Source doc | Notes |
|---|------|------------|-------|
| 1 | Approval workflow (client) | `docs/modules/clients.md` | Also in `diagrams/sd-main-features.md#d-01` |
...
| 4 | Payment approval | `docs/modules/payment.md` | `diagrams/sd-main-features.md#d-04`; before/after pair in `diagrams/workflows.md#d-01` |
...
(Master count above shows 13 *unique flows* with diagrams already
landed inside per-module pages; the rest above are duplicates of the
same flow in different consolidation pages, or are architecture
diagrams rather than feature flows.)

## sd-main inventory

### `orders`

The single most important domain in sd-main. Mostly P0.
...
80+ reports — one diagram per report would be wasteful. Author one
canonical "report run" flow plus diagrams for the few reports that
have non-trivial computation pipelines.
...
### `doctor` / `neakb` (legacy / specialised)
...
The whole module is the canonical "swap dealer connection in a loop"
pattern; one diagram for the pattern + a few for non-trivial reports.
...
## Cross-project integration flows

These are the highest-leverage diagrams to author — every one of them
spans 2 or 3 projects and is what new engineers ask about first.
...
### First batch (P0, highest-leverage) — 30 flows

1. Order create (web) — already done; refresh against orders.md only
2. Order create (mobile) — already partial
3. Order status transition diagram (full state machine) — already done
4. Defect on delivery — already done; consider tightening
5. Order export (1C / Didox / Faktura) — already done
...
11. Settlement → distribute → notify (X6) — already done
12. Phone directory sync (X7)
13. SMS round-trip across projects (X10)
14. New-dealer onboarding (X9) — already done
15. sd-cs cross-dealer report (X8) — already done
...
23. Visit + GPS geofence — already done; verify against gps3 reality
24. Out-of-zone reject — surfaces a sub-flow
25. Goods receipt — already done
26. Stock transfer + inter-filial movement (two diagrams)
27. Stocktake reconciliation + post adjustments — already done
28. Audit submission — already done
29. RBAC bind user / reload assignments
30. SMS DLR callback (sd-main + sd-billing parity)

### Second batch (P1 supporting) — 41 flows

Grouped by module owner so authoring can be done in parallel batches:
...
### Third batch (P2 edge cases) — 26+ flows

Bulk imports, archive, dbservice, retries, settings panels, single-action
controllers, legacy-flagged modules (`doctor`, `neakb`, `manager`,
sync-obsolete). These are nice-to-have for completeness and only need
diagrams when the underlying logic is unusual; CRUD-only entries can
stay textual.

## How to use this inventory

- **PMs** — pick the next batch by skimming P0/P1. The cross-project
  X-rows are the highest external-stakeholder leverage.
- **Tech leads** — confirm priorities reflect current sprint focus;
  override individual rows in PR comments.
- **Doc maintainers** — when a Mermaid block lands inside a
  module's source page, change the row's "Existing diagram?" cell to
  cite the doc + section anchor, then move the row from a backlog
  batch into the "Already done" table at the top.
- **Engineers writing the diagrams** — see
  [Workflow design standards](./team/workflow-design.md) for the 12
  principles. Pair with one of the existing diagrams in
  `docs/diagrams/sd-main-features.md` to copy the styling cookbook.

## Notes on what *isn't* in this inventory

- **Pure CRUD** — `directory/*`, the `partner/View*`, settings forms,
  classic list/edit/delete actions. No diagram value beyond a sentence.
- **Internal helpers** — `actionGetData`, `actionFetch*`, AJAX form
  preloaders. They serve a parent flow and should be diagrammed (if at
  all) inside that parent.
- **Test endpoints** — `actionTest`, `*Test.php` controllers. Not
  authoring targets.
- **Obsolete files** — anything ending `.obsolete`. Excluded outright.
- **Legacy generations** — `gps` and `gps2` actions are listed only
  where they are still in production for some tenants; new diagrams
  should target `gps3`.
