---
sidebar_position: 6
title: Schema reference (live + models)
audience: Backend engineers, QA, PM, Data engineers
summary: Authoritative schema reference for sd-main built from BOTH the live MySQL dump AND the active Yii models. Truth-source for migrations, reports, and any RAG query about the data layer.
topics: [schema, erd, models, tables, columns, indexes, mysql, charset, engine]
---

# Schema reference (live + models)

This page combines two sources:

- **`live-schema.sql`** — `mysqldump --no-data --routines --triggers` of a
  development sd-main MySQL 8.0 instance.
- **`schema-extract.json`** — parsed `@property` docblocks,
  `tableName()` / `filialTable()` / `primaryKey()` / `relations()` from
  every active Yii model under `sd-main/protected/models/`.

Run the procedure under [Refresh procedure](#refresh-procedure)
after any schema-changing PR.

## Headline numbers

| Metric | Value |
|--------|-------|
| Tables in live MySQL | **335** |
| Active models | **306** |
| Models resolvable to a real table | **296** |
| Tables in MySQL with no model | **40** |
| Total columns (live) | **4073** |
| Total declared `relations()` | **93** |
| Foreign keys at DB level | **2** |
| Indexes (non-unique) | **292** |
| Unique keys | **25** |

## Engines and charsets
...
**Action item**: any MyISAM tables and any `utf8mb3` tables are legacy.
Plan a migration to InnoDB + `utf8mb4` over time.

## Tables in DB without a Yii model

40 tables exist in MySQL but no Yii model maps to them.

| Table | Cols | Indexes | Engine | Charset |
|-------|------|---------|--------|---------|
...
| Table | Cols | PK | Engine / charset | Model |
|-------|------|----|--------------------|-------|
...
## Index of active models

All 306 active Yii models, alphabetised.

| Class | Table | PK | Cols (model) | Cols (live) | Relations |
|-------|-------|----|--------------|-------------|-----------|
...
⚠ = expected table not found in the live dump.
