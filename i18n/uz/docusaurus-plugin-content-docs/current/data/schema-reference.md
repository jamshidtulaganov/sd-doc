---
sidebar_position: 6
title: Sxema havolasi (live + modellar)
audience: Backend engineers, QA, PM, Data engineers
summary: sd-main uchun avtoritetli sxema havolasi, jonli MySQL dump VA faol Yii modellaridan ikkalasidan ham qurilgan. Migratsiyalar, hisobotlar va ma'lumot qatlami haqidagi har qanday RAG so'rovi uchun haqiqat manbasi.
topics: [schema, erd, models, tables, columns, indexes, mysql, charset, engine]
---

# Sxema havolasi (live + modellar)

Bu sahifa ikki manbani birlashtiradi:

- **`live-schema.sql`** — rivojlanish sd-main MySQL 8.0 nusxasining `mysqldump --no-data --routines --triggers`.
- **`schema-extract.json`** — `sd-main/protected/models/` ostidagi har bir faol Yii modelidan parse qilingan `@property` docblock-lari, `tableName()` / `filialTable()` / `primaryKey()` / `relations()`.

Sxema o'zgartiruvchi har qanday PR-dan keyin [Yangilash protsedurasi](#yangilash-protsedurasi) ostidagi protsedurani ishga tushiring.

## Sarlavha raqamlari

| Metrika | Qiymat |
|--------|-------|
| Jonli MySQL dagi jadvallar | **335** |
| Faol modellar | **306** |
| Haqiqiy jadvalga hal qilinadigan modellar | **296** |
| Modeli yo'q MySQL jadvallari | **40** |
| Jami ustunlar (jonli) | **4073** |
| Jami e'lon qilingan `relations()` | **93** |
| DB darajasidagi tashqi kalitlar | **2** |
| Indekslar (noyob bo'lmagan) | **292** |
| Noyob kalitlar | **25** |

## Engine-lar va belgilar to'plami
...
| Belgilar to'plami | Soni |
|---------|-------|
| `utf8mb3` | 293 |
| `utf8mb4` | 42 |

**Harakat elementi**: har qanday MyISAM jadvallari va har qanday `utf8mb3` jadvallari legacy. Vaqt o'tishi bilan InnoDB + `utf8mb4` ga migratsiyani rejalashtiring.

## Yii modeli bo'lmagan DB jadvallari

40 jadval MySQL da mavjud, lekin ularga hech qanday Yii modeli mos kelmaydi.

| Jadval | Ust | Indekslar | Engine | Charset |
|-------|------|---------|--------|---------|
...
## Domen bo'yicha jadvallar

### AKB (3 jadval)

| Jadval | Ust | PK | Engine / charset | Model |
|-------|------|----|--------------------|-------|
...
## Faol modellar indeksi

Barcha 306 ta faol Yii modeli, alfavit tartibida.

| Klass | Jadval | PK | Ust (model) | Ust (live) | Munosabatlar |
|-------|-------|----|--------------|-------------|-----------|
